...
public abstract class JsonTokenTestBase extends TestCase {
  ...
  protected void setUp() throws Exception {
    ...
    clock.setNow(Instant.ofEpochSecond(1276669722L));
  }
  ...
  protected void assertPayload(JsonToken token) {
    ...
    assertEquals(Instant.ofEpochSecond(1276669722L), token.getIssuedAt());
    assertEquals(Instant.ofEpochSecond(1276669723L), token.getExpiration());
  ...
  /**
   * Deserializes the token string such that tokenStrings without signatures are allowed. Only
   * supports token strings with at least two parts.
   */
