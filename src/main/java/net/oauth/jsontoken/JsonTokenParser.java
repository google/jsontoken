...
public class JsonTokenParser extends AbstractJsonTokenParser {
  ...
  /**
   * Creates a new {@link JsonTokenParser} with a default system clock. The default system clock
   * tolerates a clock skew of up to {@link SystemClock#DEFAULT_ACCEPTABLE_CLOCK_SKEW}.
   *
   * @param verifierProviders an object that provides signature verifiers based on a signature
   *     algorithm, the signer, and key ids.
   ...
   * VerifierProviders to obtain the secret key.
   *
   * @param jsonToken
   ...
   *     valid verifier for the issuer or if the header does not exist
   */
   ...
   * @return the deserialized {@link JsonObject}, suitable for passing to the constructor of {@link
   *     JsonToken} or equivalent constructor of {@link JsonToken} subclasses.
   * @throws SignatureException when the signature is invalid or if any of the checkers fail
   ...
   * AbstractJsonTokenParser#verifyInternal(JsonToken, List)}
   *
   ...
   * AbstractJsonTokenParser#signatureIsValidInternal(String, List)}.
   *
   ...
   * @throws IllegalArgumentException if the signature algorithm is not supported
   * @throws IllegalStateException if there is no valid verifier for the issuer or if the header
   *     does not exist
   */
